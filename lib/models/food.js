...
class Food {
    static all() {
        ...
        .then( (response) => {
            if (document.title === "Quantified Self - Manage Foods") {var page = 'foods'}
            if (document.title === "Quantified Self - Diary") {var page = 'diary'}
            response.forEach( (food) => {
                const foodRowTemplate = HTMLHelper.toHtml(food.name, food.calories, food.id, page);
