...
/***/ (function(module, exports, __webpack_require__) {
	...
	const HTMLHelper = __webpack_require__(3);
	const setUrl = __webpack_require__(5).setUrl;
	const AjaxRequests = __webpack_require__(4);
	const SortingHelper = __webpack_require__(6);

	class Food {

	    static all() {
	        AjaxRequests.getFoods().then(response => {
	    ...
	    static postNewFood(foodName, foodCalorie) {
	        ...
	        $.post(`${setUrl}api/v1/foods`, data).then(response => {
	        ...
	        }).then(HTMLHelper.resetForm()).catch(() => {
	...
	class HTMLHelper {
	  ...
	  static insertFoodMealRelationsInHTML() {
	    ...
	    mealNames.forEach(mealName => {
	      ...
	      AjaxRequests.getFoodMealRelationships(mealId).then(meal => {
...
/***/ (function(module, exports) {
	...
	module.exports = {
	  setUrl: 'http://whispering-earth-58532.herokuapp.com/'
	};
	...
	class SortingHelper {
	  static quickSort(foods) {
	    ...
	    if (foodsLength <= 1) {
	      ...
	      return foods;
	    } else if (!!foods.reduce((a, b) => {
	      return a.calories === b.calories ? a : NaN;
	    })) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	class Meal {
	  ...
	  static handleDeleteMealFoods() {
	    $('.meal').on('click', '.delete', event => {
	      const foodId = event.target.dataset.id;
	      const mealId = $(event.target).parent().parent().parent().parent()[0].dataset.id;
	      const mealName = $(event.target).parent().parent().parent().parent()[0].dataset.name;
	      AjaxRequests.deleteMealFood(mealId, foodId).then(() => {
	        HTMLHelper.removeMealFoodsDisplay(mealName);
	      }).then(() => {
	        HTMLHelper.insertFoodMealRelationsInHTMLFor(mealName);
	        Meal.setCalories(mealName);
	        Meal.setTotalCalories();
	      });
	    });
	  ...
	  static setTotalCalories() {
	    AjaxRequests.getMeals().then(meals => {
	  ...
	  static populateMealPage() {
	    return AjaxRequests.getMeals().then(meals => {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	$(document).ready(function () {
	  ...
	  Meal.populateMealPage().then(() => {
	    Meal.handleDeleteMealFoods();
	  });
	});
